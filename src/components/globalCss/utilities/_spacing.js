...
export const spacingStyles = makeStyles(({ palette, ...theme }) => ({
    '@global': {
        ...
        '.hover-ml-1': { transition: 'all 250ms' },
        '.hover-ml-1:hover': { marginLeft: '0.5rem !important' },
        '.hover-mr-1': { transition: 'all 250ms' },
        '.hover-mr-1:hover': { marginRight: '0.5rem !important' },
        '.hover-pl-1': { transition: 'all 250ms' },
        '.hover-pl-1:hover': { paddingLeft: '0.5rem !important' },
        '.hover-pr-1': { transition: 'all 250ms' },
        '.hover-pr-1:hover': { paddingRight: '0.5rem !important' },
        ...
        '.min-w-0': { minWidth: '0 !important' },
        '.min-h-0': { minHeight: '0 !important' },
...
const generateHeightWidh = (start = 100, end = 1200, increament = 5, unit = 'px') => {
    let classList = {};
    const unitName = unit === 'px' ? 'px' : '';

    for (let i = start; i <= end; i += increament) {
        classList[`.w-${i}${unitName}`] = {
            width: `${i}${unit} !important`
        };
        classList[`.min-w-${i}${unitName}`] = {
            minWidth: `${i}${unit} !important`
        };
        classList[`.max-w-${i}${unitName}`] = {
            maxWidth: `${i}${unit} !important`
        };
        classList[`.h-${i}${unitName}`] = {
            height: `${i}${unit} !important`
        };
        classList[`.min-h-${i}${unitName}`] = {
            minHeight: `${i}${unit} !important`
        };
        classList[`.max-h-${i}${unitName}`] = {
